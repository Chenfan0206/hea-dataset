...
This node classifies HEA papers along three axes (base_material, application,
research_paradigm) and loads the corresponding HEA-safe supplement files.
"""
...
_DOMAIN_MAP = {
    # Public release keeps HEA-related overlays only. Missing supplement files
    # are safely ignored by load_routing_supplement.
    "Additive_Manufacturing": "domain/Additive_Manufacturing.txt",
    "Laser_Assisted": "domain/Laser_Assisted.txt",
    "Interface_Dominated": "domain/Interface_Dominated.txt",
}
...
def detect_sub_field(state: KnowMatState) -> dict:
    ...
    # HEA public release: do not expose or load non-HEA material directions.
    routing["base_material"] = "Metals"
    routing["domain_overlays"] = [
        overlay
        for overlay in routing.get("domain_overlays", [])
        if overlay in _DOMAIN_MAP
    ]
    patch_tags = routing.get("patch_tags", [])
    if "High_Entropy" not in patch_tags:
        patch_tags.append("High_Entropy")
    routing["patch_tags"] = patch_tags
