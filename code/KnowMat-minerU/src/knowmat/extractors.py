...
class PaperRouting(BaseModel):
    ...
    Classifies a high-entropy alloy paper along three axes:
    base_material, application, and research_paradigm.
    """

    base_material: str = Field(
        description=(
            "Primary material system. This public release is HEA-scoped and uses Metals."
        )
    )
    ...
    domain_overlays: List[str] = Field(
        ...
        description=(
            "Optional HEA-related domain overlays. Possible values: "
            "Additive_Manufacturing, Laser_Assisted, Interface_Dominated."
        )
    )
    patch_tags: List[str] = Field(
        default_factory=list,
        description=(
            "Optional HEA patch tags. E.g. High_Entropy, Additive_Manufacturing, "
            "Laser_Assisted, Directional_Anisotropy, Interface_Dominated."
        )
    )
...
class CompositionProperties(BaseModel):
    ...
    sample_id: Optional[str] = Field(
        ...
        description=(
            ...
            "When splitting by time/temperature conditions, encode in sample_id (e.g., HEA_700C_50h, Aged_700C_2h).\n"
...
class CompositionList(BaseModel):
    ...
    compositions: List[CompositionProperties] = Field(
        description=(
            ...
            "• HEA feedstock powder or precursor alloy states with independent characterization\n"
